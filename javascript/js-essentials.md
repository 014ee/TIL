# ✅ JS의 역사
* `1996년 3월` 넷스케이프 내비게이터 2에 모카라는 이름으로 탑재
* `1996년 8월` MS에서 자바스크립트 파생 버전인 JS스크립트를 IE 3에 탑재 (두 회사의 기능 경쟁으로 크로스브라우징 이슈)
* `1996년 9월` 라이브스크립트로 이름 변경
* `1996년 11월` 넷스케이프가 컴퓨터 시스템 표준을 관리하는 비영리 표준화 기구 ECMA 인터내셔널에 자바스크립트 표준화 요청
* `1996년 12월` 자바스크립트로 최종 이름 변경
* `1996년 7월` ECMA-262라는 표준화된 자바스크립트 초판의 명세 완성 (이후 상표권 문제로 ECMA스크립트로 명명됨)
* `1999년` ECMA스크립트 3 공개 (정규표현식, try...catch 예외 처리) 
* `2009년` HTML 5와 함께 ES 5 (JSON, strict mode, 접근자 프로퍼티, forEach, map, filter, reduce, some, every)
